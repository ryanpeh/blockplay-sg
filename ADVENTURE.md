...
Marina starts with **Waterfront** as its active objective. The companion accepts typed requests or GPT-Live-1 speech, asks GPT-5.6 Luna for a structured destination ID, and applies the proposal through the same local game interface. Try “give me something closer”, “take me to the museum”, or “skip this stop”. The HUD names the objective and the minimap marks it purple. Raffles Place and Queenstown provide education-only guides; their existing exploration and collection behavior is unchanged.
...
- `pnpm test:adventure --live`: first seven text requests use real Luna (four learning across three regions, three travel); failure/voice checks remain explicitly mocked.
...
Regional rollout verification: 113 unit tests and production build pass. Live Luna returned the Queenstown library and Boat Quay cards from their respective game panels without changing stamps. Both new panels also passed simulated GPT-Live delegation checks, mobile screenshot review and reset cancellation tests. This does not replace a physical-microphone listening test. `RegionGuide.tsx` plus `learning-guide.ts` provide the read-only game adapter; backend travel proposals are disabled outside Marina.

All three regions have educational companions sharing 18 curated topics (six per region). Try “tell me about this stop”, “tell me about the museum”, “Marina Bay highlights”, “what is special about Queenstown?” or “tell me about Raffles Place”. These use the same text/voice pipeline, but return a read-only learning card, official source link and reflection prompt. “Take me to the museum” still changes the objective. Stamps are preserved in either case. Raffles Place and Queenstown now host education-only panels, with six cards each covering all 19 regional stops. “About this stop” uses the nearest game stop when no active objective exists. Stop-to-topic links are thematic, not surveyed landmarks.

Facts and source links live in `src/data/singapore-guide.ts`, `raffles-guide.ts` and `queenstown-guide.ts`, reviewed 13 September 2026 against PUB, Singapore Tourism Board, Gardens by the Bay, Esplanade and National Heritage Board pages. Astra selects only existing topic IDs; it does not write the facts. Unknown topics and questions not supported by the cards (including current hours/prices) receive an explicit limitation, not invented information. Active/nearest game-stop context is supplied, but this is not camera recognition or accurate geographic positioning. Labels distinguish the stylised game from real places.

Educational regression coverage includes source-card rendering, unchanged objectives/stamps, rejected topic IDs, stale/reset results and simulated spoken education. `pnpm test:adventure --live` now includes seven live Luna requests (four educational across all regions and three travel); remaining failure/voice checks are mocked. The prior live synthetic voice test verified the shared transport; educational speech and physical microphone quality still need a human listening check.
