...
export async function withBudget(task, ledgerPath = resolve('reconstruction/api-usage.json')) {
  ...
  try {
    ...
    const request = async (kind, callback) => {
      if (isStaticAttempt(kind) && ledger.attempts.filter(entry => isStaticAttempt(entry.kind)).length >= ledger.limit) throw new Error(`Local API budget exhausted (${ledger.limit}).`);
      const isCapture = value => value.startsWith('image');
      if (isCapture(kind) && ledger.imageAllowance) {
      ...
      try {
        ...
        await save();
        console.log(`Recorded events: ${ledger.attempts.length}; Static attempts: ${ledger.attempts.filter(entry => isStaticAttempt(entry.kind)).length}/${ledger.limit}`);
        return result;
      } catch (error) {
