...
try {
 ...
 await wait(`Number(document.querySelector('.fps-score').firstChild.textContent.split('/')[0])>=1`,25000);
 await screenshot('pilot-first-kill');
 await key('Escape','Escape'); await wait(phase('paused'));
 assert.equal(await evaluate(`document.querySelector('.fps-pilot-panel').dataset.pilotActive`),'false');
 const paused=await evaluate(ammo); await delay(500); assert.equal(await evaluate(ammo),paused);
 await click(button('Watch AI play')); await wait(phase('playing'));
 await click(button('Take control')); await wait(`!!document.pointerLockElement`);
 assert.equal(await evaluate(`document.querySelector('.fps-pilot-panel').dataset.pilotActive`),'false');
 await key('Escape','Escape'); await wait(phase('paused'));
 await click(button('Watch AI play')); await wait(phase('playing'));
 await wait(phase('complete'),90000); await screenshot('pilot-range-clear');
 assert.deepEqual(errors,[]); console.log('PASS: local pilot kills all eight targets through player controls, pause and pointer-locked takeover.');
