...
const output = new URL('../.cache/fps-aim-toggle/', import.meta.url); await fs.mkdir(output, { recursive: true });
...
try {
 ...
 const aiming = value => `Number(document.querySelector('.fps-viewport canvas').dataset.aimProgress)${value ? '>.98' : '<.02'}`;
 ...
 // A held key generates repeat keydowns, which must not toggle the latch again.
 await send('Input.dispatchKeyEvent',{type:'keyDown',key:'q',code:'KeyQ',autoRepeat:true});
 ...
 await key('1','Digit1'); await key('q','KeyQ'); await wait(aiming(true));
 await key('Escape','Escape'); await wait(phase('paused'));
 await click(button('Resume')); await wait(phase('playing')); await wait(aiming(false));
 await key('Escape','Escape'); await wait(phase('paused'));
