...
export const liveSession = {
  ...
  instructions: 'You are Blockplay’s concise educational Singapore companion. Delegate every objective request AND every question about places, history, highlights or features to the client backend. Never answer factual questions from memory. Speak only the source-grounded commentary returned by the app; do not add facts. Never select a destination or claim an objective changed yourself. Wait for the verified application result before confirming it. Then listen. Do not invent routes. The app uses Astra to interpret requests. Ask the player to repeat if speech is unclear.',
};
...
export async function interpret(text: string, state: AdventureSnapshot, key: string, base: string, fetcher = fetch) {
  ...
  const response = await fetcher(`${base}/responses`, {
    ...
    body: JSON.stringify({ model: 'gpt-6-astra', store: false, reasoning: { effort: 'low' }, max_output_tokens: 1200,
  ...
  if (!response.ok) throw new Error('Astra request failed. Check server credentials and model access.');
  ...
  try { decision = JSON.parse(output); } catch { throw new Error('Astra returned an invalid objective.'); }
  if (decision && typeof decision === 'object' && 'intent' in decision && decision.intent === 'learn') {
    if (!('destinationId' in decision) || decision.destinationId !== null || !('topicId' in decision) || (decision.topicId !== null && !learningTopic(decision.topicId))) throw new Error('Astra returned an invalid learning topic.');
    return { intent: 'learn' as const, destinationId: null, topicId: decision.topicId as string | null };
  }
  if (!validateDecision(decision) || (decision.destinationId !== null && !state.destinations.some(d => d.id === decision.destinationId))) throw new Error('Astra returned an invalid objective.');
  return decision;
}
...
export function createAdventureHandler(env: Record<string, string | undefined>, fetcher = fetch) {
  ...
  return async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      if (!key) return reply(503, { error: 'Set OPENAI_API_KEY on the server to enable Astra and GPT-Live-1. You can keep exploring.' });
