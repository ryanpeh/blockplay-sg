...
export default function FpsWeaponHud({ hud, weapon, canFight }: { hud: FpsHud; weapon: WeaponSpec; canFight: boolean }) {
  ...
  return <div className="fps-weapon-hud" data-aiming={hud.aiming} data-reloading={reloading}
    ...
    {canFight && <div className={`fps-crosshair fps-tactical-crosshair ${hud.aiming ? 'aiming' : ''}`} aria-hidden="true"><i /><i /><i /><i /><b /><span className="fps-ads-dot-ring" /></div>}
