...
import { HAS_LAN_SERVER } from '../lib/deployment';
...
export default function LanLobby({ onLaunch, onBack }: { onLaunch: (session: LanSession, bots: number, squad: Squad) => void; onBack: () => void }) {
  const [mode, setMode] = useState<Mode>(() => !HAS_LAN_SERVER ? 'solo' : invitedRoom() ? 'join' : 'host');
  ...
  useEffect(() => {
    alive.current = true;
    let ignore = false;
    if (HAS_LAN_SERVER) void lanServerAvailable().then(available => { if (!ignore) setServer(available); }).catch(() => { if (!ignore) setServer(false); });
  ...
  const chooseMode = (next: Mode) => {
    if (!HAS_LAN_SERVER && next !== 'solo') return;
    if (next === mode) return;
  ...
  const connect = async () => {
    ...
    if (!HAS_LAN_SERVER) { setError('Multiplayer host/join is unavailable in this playable demo. Choose Solo vs bots.'); return; }
  ...
  return <section className="lan-lobby" aria-label="Multiplayer lobby">
    <header className="lan-header"><div><span className="lan-kicker">BLOCKPLAY / LOCAL OPERATIONS</span><h2>{!HAS_LAN_SERVER ? 'Take on the bots.' : 'Never deploy alone.'}</h2><p>{!HAS_LAN_SERVER ? 'Solo matches are ready. Multiplayer host/join is unavailable in this playable demo.' : 'Marina Bay. Your squad. A few uninvited bots.'}</p></div><span className="lan-network-badge"><Wifi size={15} /> {!HAS_LAN_SERVER ? 'SOLO' : 'P2P / LAN'}</span></header>
    <div className="lan-layout">
      <main className="lan-main">
        <div className="lan-mode-picker" aria-label="Connection mode">
          {([{ id: 'host', icon: Radio, title: 'Host LAN', detail: 'Create a room' }, { id: 'join', icon: Users, title: 'Join LAN', detail: 'Enter a room code' }, { id: 'solo', icon: Bot, title: 'Solo vs bots', detail: 'No connection needed' }] as const).map(option => <button key={option.id} disabled={!HAS_LAN_SERVER && option.id !== 'solo'} aria-pressed={mode === option.id} onClick={() => chooseMode(option.id)}><option.icon size={23} /><strong>{option.title}</strong><small>{!HAS_LAN_SERVER && option.id !== 'solo' ? 'Unavailable in this demo' : option.detail}</small></button>)}
        </div>
