import { canOccupy, MARINA_BOUNDS, type Obstacle } from './marina-collision';
...
export const createVehicle = (kind: VehicleKind): VehicleState => ({ kind, ...VEHICLE_SPAWNS[kind], y: .13, speed: 0, climb: 0 });
...
export function driveVehicle(v: VehicleState, forward: number, steer: number, brake: boolean, dt: number, obstacles: readonly Obstacle[]) {
  dt = clamp(dt, 0, .05);
  ...
  const clear = (x: number, z: number, heading: number) => {
    ...
    return b.minX >= MARINA_BOUNDS.minX && b.maxX <= MARINA_BOUNDS.maxX && b.minZ >= MARINA_BOUNDS.minZ && b.maxZ <= MARINA_BOUNDS.maxZ && !obstacles.some(o => b.maxX > o.minX && b.minX < o.maxX && b.maxZ > o.minZ && b.minZ < o.maxZ);
  };
...
export function canFly(x: number, y: number, z: number, obstacles: readonly FlightObstacle[]) {
  const r = 2.5, bounds = MARINA_BOUNDS;
...
export function flyVehicle(v: VehicleState, forward: number, steer: number, lift: number, boost: boolean, dt: number, obstacles: readonly FlightObstacle[]) {
  dt = clamp(dt, 0, .05);
  ...
  for (let i = 0; i < steps; i++) {
    ...
    if (canFly(x, nextY, z, obstacles)) y = nextY; else { climb = 0; blocked = true; }
    if (canFly(x + dx / steps, y, z, obstacles)) x += dx / steps; else { speed = 0; blocked = true; }
    if (canFly(x, y, z + dz / steps, obstacles)) z += dz / steps; else { speed = 0; blocked = true; }
  }
  if (y === .13 || y === 120) climb = 0;
  return { state: { ...v, x, y, z, yaw, speed, climb }, blocked };
}
export function vehicleExit(v: VehicleState, obstacles: readonly Obstacle[]) {
  ...
  for (const angle of [Math.PI / 2, -Math.PI / 2, Math.PI, 0]) {
    ...
    if (canOccupy(x, z, .38, [...obstacles, vehicleBounds(v)])) return { x, z };
  }
  return null;
}
