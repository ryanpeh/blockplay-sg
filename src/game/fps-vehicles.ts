...
import { createVehicle, driveVehicle, flyVehicle, vehicleBounds, vehicleExit, VEHICLE_SPAWNS, type FlightObstacle, type VehicleKind } from './vehicle-rules';
import type { Obstacle } from './marina-collision';

export function createFpsVehicles(scene: THREE.Scene, obstacles: Obstacle[], skins: Record<VehicleKind, string>) {
  const root = new THREE.Group(); scene.add(root);
  const states = { car: createVehicle('car'), helicopter: createVehicle('helicopter') };
  ...
  pad.rotation.x = -Math.PI / 2; pad.position.set(VEHICLE_SPAWNS.helicopter.x, .145, VEHICLE_SPAWNS.helicopter.z); root.add(pad);
  ...
  function interact(player: { x: number; z: number }) {
    if (active) {
      const point = vehicleExit(states[active], footObstacles(active));
  ...
  function step(keys: ReadonlySet<string>, dt: number) {
    ...
    const result = active === 'car' ? driveVehicle(states.car, forward, steer, keys.has(' '), dt, footObstacles('car')) : flyVehicle(states.helicopter, forward, steer, Number(keys.has(' ')) - Number(keys.has('c') || keys.has('control')), keys.has('shift'), dt, airObstacles());
  ...
  return {
    ...
    reset() { active = null; notice = ''; noticeTime = 0; states.car = createVehicle('car'); states.helicopter = createVehicle('helicopter'); models.helicopter.rotation.x = 0; sync(0); },
