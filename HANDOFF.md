...
**Audition blocker confirmed (2026-09-13):** ElevenLabs User Read now works. Voice Design API returned `403 feature_unavailable`: creating a voice through the API requires a paid plan, despite free Voice Design being listed in the website product. No preview generated; first denial left account usage at zero. Preserved failed attempts under `.cache/encik-audition/93f198a2b596/`; do not delete markers or keep retrying the same plan restriction. The tool now captures/sanitizes provider errors; four mocked tests pass. User authorized free-only, two-line audition. Website fallback is the next step; native Safari access was blocked by pending macOS Computer Use permissions. See `docs/ENCIK-AUDITION.md`.

**Pending: two-line ElevenLabs audition (2026-09-13).** User authorized only a free-tier audition before approving a full callout batch. `pnpm voice:audition` dry-runs the exact 118-character script; `pnpm voice:audition --generate` verifies Free-tier quota and makes one Voice Design v3 request, caching returned previews and an HTML listening page. A permanent attempt marker prevents accidental retries after timeouts. Three mocked tests pass (`node --test scripts/encik-audition.test.mjs`). No ElevenLabs key was configured and no real audio was generated or credits spent at preparation time. User was asked to add `ELEVENLABS_API_KEY` to `.env.local` and reply ready. Continue with that one request when available, then audition with the user before saving a voice or generating the full pack. See [audition setup](docs/ENCIK-AUDITION.md).
...
Read `ADVENTURE.md` before changing text/voice behavior. `pnpm server` loads private `OPENAI_API_KEY` and starts the two API endpoints on loopback 3001; `pnpm dev` proxies them. Never put the key in a VITE variable. Requested models are exactly `gpt-6-astra` and `gpt-live-1`; preserve the separate Live protocol and shared local apply logic. No new dependencies. Game geometry, routes and Google image cache/ledger are unchanged.

`adventure.ts` owns objective IDs, request/revision/session guards and deterministic closer selection; `MarinaGame.tsx` feeds position/collection and draws active HUD/minimap. Reset remounts the panel and invalidates requests. Objective changes must never mutate collected stamps or bypass existing arrival checks. `adventure-client.ts` is shared by text and voice, and only acknowledges a game-applied update. `live-voice.ts` owns microphone/WebRTC/delegation/cleanup; never run actions from transcript-gap heuristics. Origin checks are not authentication: add access control before public deployment. Browser QA commands and synthetic-audio caveats are in `ADVENTURE.md`.
