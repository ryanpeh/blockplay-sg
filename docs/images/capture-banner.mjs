...
const views=[{id:'marina-banner',module:'marina',builder:'buildMarinaScene',camera:[-100,85,150],target:[95,32,-90]}];
...
try{
 ...
 const renderer=new THREE.WebGLRenderer({antialias:true,preserveDrawingBuffer:true});renderer.setSize(1600,900);renderer.setPixelRatio(1);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=.9;renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;document.body.appendChild(renderer.domElement);
 const camera=new THREE.PerspectiveCamera(55,1600/900,.1,1400);camera.position.set(${view.camera});camera.lookAt(${view.target});world.animate?.(0);renderer.render(world.scene,camera);window.__evidenceWorld=world;window.__evidenceRenderer=renderer;
 return {image:renderer.domElement.toDataURL('image/png'),children:world.scene.children.length};})()`});
 if(result.exceptionDetails)throw Error(JSON.stringify(result.exceptionDetails));
 await writeFile(`docs/images/blockplay-sg-marina.png`,Buffer.from(result.result.value.image.split(',')[1],'base64'));console.log(view.id,'rendered');
 }
 await writeFile('docs/images/render-settings.json',JSON.stringify({settingsWrittenAt:new Date().toISOString(),method:'Current scene builders rendered with documentation cameras; not player-camera screenshots or reconstruction output.',viewport:[1600,900],fov:55,views},null,2)+'\n');
